...
async function detectFace1(imgarrs, threshold){
    //导入Pnet预训练模型
    const Pnet = await tf.loadGraphModel('file://./public/model/Pnet1/model.json');
    const Rnet = await tf.loadGraphModel('file://./public/model/Rnet1/model.json');
    const Onet = await tf.loadGraphModel('file://./public/model/Onet1/model.json');
    let imgarr = imgarrs.arraySync()
    let caffe_img = imgarr.map(val=>{
        return val.map(val=>{
            return val.map(val=>{
