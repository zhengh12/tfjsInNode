...
function inception_resnet_stem(input){
    ...
    const m =  tf.concat([c1,c2],channel_axis)
    ...
    const m2 = tf.concat([c1,c2],channel_axis)
    ...
    let m3 = tf.concat([p1,p2],channel_axis)
...
function inception_resnet_v2_A(input, scale_residual=True){
    ...
    const irMerge = tf.concat([ir1,ir2,ir3], channel_axis)
...
function inception_resnet_v2_B(input, scale_residual=True){
    ...
    const irMerge = tf.concat([ir1,ir2],channel_axis)
...
function reduction_A(input, k=192, l=224, m=256, n=384){
    // if K.image_dim_ordering() == "th":
    //     channel_axis = 1
    // else:
    //     channel_axis = -1
    ...
    let m = tf.concat([r1,r2,r3],channel_axis)
    // m = BatchNormalization(axis=1)(m)
    // m = Activation('relu')(m)
    m = tf.layers.batchNormalization({axis:1}).apply(m)
...
function reduction_resnet_v2_B(input){
    ...
    let m = tf.concat([r1,r2,r3,r4],channel_axis)
...
function create_inception_resnet_v2(nb_classes=1001, scale=true){
    ...
    let init = true ? tf.input({shape:[3,299,299]}) : tf.input({shape:[299,299,3]})
