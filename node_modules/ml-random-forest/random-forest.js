...
function featureBagging(trainingSet, n, replacement, seed) {
  ...
  let toRet = new Matrix__default(trainingSet.rows, n);
...
class RandomForestBase {
  ...
  predict(toPredict) {
    ...
    for (let i = 0; i < this.nEstimators; ++i) {
      let X = new Matrix.MatrixColumnSelectionView(toPredict, this.indexes[i]); // get features for estimator
      predictionValues[i] = this.estimators[i].predict(X);
    }

    predictionValues = new Matrix.MatrixTransposeView(
      new Matrix.WrapperMatrix2D(predictionValues),
    );
